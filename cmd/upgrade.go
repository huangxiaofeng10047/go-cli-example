...
func newUpgradeCmd(settings *cli.EnvSettings, cfg *action.Configuration, out io.Writer, debug action.DebugLog) *cobra.Command {
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			if client.Install {
				...
				if err == driver.ErrReleaseNotFound || isReleaseUninstalled(versions) {
					...
					rel, err := runInstall(settings, args, instClient, valueOpts, out, debug)
					if err != nil {
						return err
					}
